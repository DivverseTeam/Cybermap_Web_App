	...
	return (
		...
		>
			{!["home", "actions"].includes(groupName.toLocaleLowerCase()) && (
				<span className="mt-[-16px] font-[500] text-[#B9BBC6] text-sm uppercase [@media(min-width:1300px)]:mt-[-10px]">
					{groupName}
				</span>
			)}
			{groupName.toLocaleLowerCase() === "actions" && (
				<div className="mt-4 2xl:mt-11 [@media(min-width:1300px)]:mt-8"></div>
			)}
				...
				{links.map((link: LinkProp, index) => {
					...
					return (
						<div className="relative flex items-center" key={index}>
							{isActive && (
								<div className="absolute left-[-16] h-9 w-1 rounded-md bg-primary 2xl:w-1.5 [@media(min-width:1300px)]:left-[-25]"></div>
							)}
							<Link
								key={index}
								href={`${link.href}`}
